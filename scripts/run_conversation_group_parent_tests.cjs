// Compile the production parent policy and pure Kotlin regressions without Gradle or Android.
...
const output = fs.mkdtempSync(path.join(os.tmpdir(), 'hchat-conversation-parent-tests-'));
const artifact = path.join(output, 'parents.jar');
...
run(['-Xmx256m', '-cp', compiler.join(path.delimiter), 'org.jetbrains.kotlin.cli.jvm.K2JVMCompiler',
    ...
    'app/src/main/java/h/Hchat/hooks/items/conversationgroup/ConversationGroupParentPolicy.kt',
    'scripts/tests/conversation_group_parent/ParentPolicyRegression.kt']);
run(['-cp', [artifact, stdlib].join(path.delimiter),
    'h.Hchat.hooks.items.conversationgroup.ParentPolicyRegressionKt']);
