...
模型选择页和 Agent 配置页新增“推理强度”，与模型 ID 一起按模型配置档案保存。旧配置缺少该字段时使用“默认”，不额外发送推理参数；非默认档位按接口协议发送，并只提供当前已适配模型的档位。兼容接口的实际能力仍由模型和服务端决定，不支持时显示接口错误，不会静默去掉推理参数后假装成功。配置切换会恢复各自的推理强度；运行中应用模型设置只作用于后续请求，当前已启动的请求保持原配置。测试连接、正常聊天与上下文压缩复用相同的参数适配入口。接口适配中，OpenAI 兼容接口发送 `reasoning_effort`，OpenRouter 发送 `reasoning.effort`，Anthropic 发送 `output_config.effort`，Gemini 发送 `generationConfig.thinkingConfig.thinkingLevel`；DeepSeek 和硅基流动仅对已核验模型组合相应思考开关与 effort。Anthropic effort 控制回答与推理投入，不额外开启需要签名回放的思考块；OpenRouter Claude 因当前未保存完整签名推理历史，暂只提供默认档位。原生接口的未知模型保留默认，不猜测 token 预算或扩展档位。
