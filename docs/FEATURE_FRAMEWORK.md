...
- 本分支只支持 `arm64-v8a` 和 64 位微信进程，不再支持 ARM32 / 32 位微信。Native 库只维护 `arm64-v8a`；`libsilk_codec.so`、`libhchat_crash.so` 放在 `app/src/main/jniLibs/arm64-v8a/`，Gradle 依赖中的 native 库也只打包这一 ABI。
- `libsilk_codec.so` 使用上游 1.0.6 的 16KB ELF LOAD 段对齐产物，`arm64-v8a` 的段对齐为 `0x4000`；16KB 兼容由链接器页大小参数保证，不再对普通音频缓冲区强制执行 16KB 堆内存对齐。
- `libhchat_crash.so` 由 `scripts/build_crash_native.sh` 使用 Termux Clang/LLD 生成；脚本只生成 `arm64-v8a`、去除调试符号，并校验 ELF 架构、`libc.so` 依赖和 JNI 导出。日常 Gradle/GitHub 构建直接打包已提交的 `.so`，不要求额外安装 NDK。
- 当前本地接管版本：`1.0.6`（上游提交 `c09fc1f534c7435c7b4c11a84a4632c6d177e1b9`）

## 微信 APK 横向验证

不确定微信内部逻辑时，优先对多个版本 APK 做横向对比，不要只看单一版本，也不要写死某个版本的混淆类名、字段名或构造签名。凡是涉及微信内部结构、布局、DexKit、反射、数据库、Intent、网络场景或版本兼容的改动，默认尽量覆盖 `8.0.49`、`8.0.58`、`8.0.66`、`8.0.68`、`8.0.72`、`8.0.74`、`8.0.76`、`8.0.77`、`8.0.78`；除非对应 APK 缺失或用户明确要求单版本，并且必须在结论里说明未覆盖原因。
...
export WECHAT_APK_8078="/你的路径/微信8078.apk"
```
...
- `8.0.78`: 新增横向验证目标。APK 路径通过本机变量 `WECHAT_APK_8078` 自行配置，不固定目录或文件名；versionCode、clientVersion 和 ABI 以实际 APK 检查为准。本次仅登记版本，尚未进行该版本的功能逆向或真机兼容验证，已有“已验证”版本记录不自动扩展到 8.0.78。
...
脚本杂项和音频薄封装仍然保留：`reloadPlugin()`、`compileSnapshot(String path)`、`evalSnapshot(String path)`、`evalSnapshot(InputStream inputStream)`、`evalSnapshot(byte[] data)`、`eval(String code)`、`loadJava(String path)`、`loadDex(String path)`、`loadSo(String path)`、`loadSo(String path, ClassLoader loader)`、`getDuration(String filePath)`。`reloadPlugin()` 异步重载当前插件；`compileSnapshot(String path)` 会把指定脚本编译成同目录下的 `.bshs` 加密快照文件并返回快照绝对路径；`evalSnapshot` 使用模块内置兼容 AES 密钥加载并执行 `.bshs` 快照，支持路径、输入流和字节数组三种来源，快照可在不同插件目录间加载；`compileSnapshot` / `evalSnapshot` 支持在脚本顶层和 `onLoad()` 中调用，不依赖插件完成加载登记；如果 `main.java` 只是入口包装，真正回调定义在 `eval(String code)`、`evalSnapshot()` 或 `loadJava()` 载入的文件里，加载后也必须刷新并识别这些回调；`loadJava` 支持绝对路径，相对路径按当前插件目录解析；`loadDex` 支持 dex/jar/apk，路径规则同 `loadJava`，会复制到微信私有 `code_cache` 并设为只读后用 `DexClassLoader` 加载并返回 `ClassLoader`；`loadSo` 仅接受 64 位微信进程中的 `arm64-v8a` ELF，校验通过后按内容哈希复制到 `code_cache/hchat_plugin_native/<pluginId>/` 并设为只读，通过 Android Native 加载入口绑定指定 `ClassLoader`，默认绑定微信宿主加载器。BeanShell 动态 JNI 类必须把 `NativeClass.class.getClassLoader()` 传给双参数重载；JNI 类来自 `loadDex` 时必须传其返回值；顶层 `native` 函数没有 JNI 类载体，运行时会明确拒绝调用；同一路径、同内容和同一加载器在进程内复用。替换 SO 后重新加载插件并传入新 JNI 类的加载器时，会通过独立只读副本热加载新版本；固定宿主加载器或旧加载器不能可靠热更新。插件关闭不会卸载 SO，旧版本驻留到微信进程结束；`getDuration` 返回毫秒，失败返回 `0`。
...
- 专属屏蔽规则只取该名单自身的消息类型及关键词，不再依赖此前关联模板是否启用或仍存在；启用的专属排除同样独立于旧模板并优先于专属屏蔽，关闭的排除不生效，普通模板绑定仍只使用启用且存在的模板。重新从联系人或群成员选择器选择已有名单时，只更新显示名称及规范化成员键，必须保留 `enabled/action/templateIds/customRules/typeAll/types/textKeywords/quickBlockAll`，避免保存后变成空规则。已经在旧版保存丢失的配置无法凭消息内容恢复，需要用户重新设置受影响名单。
- 本轮静态核验 8.0.49/58/66/68/72/74/76/77/78 的 `dkAddMsg` 与 `processAddMsg` 均位于同一 foundation 类，`processAddMsg` 为首参数直接传入 AddMsg 的 `void` 方法，未发现第二个被漏挂的入口；因此本次不更换 Hook、不增加消息到库后删除或全局数据库拦截。8.0.58 AddMsg 的 `p` 是二进制容器、`s` 才是消息来源，公共解析器改为只接受内部 `d` 为字符序列的文本容器，避免将二进制长度 `0` 或数字误当 `msgSource`，从而丢失真实艾特信息。
- 屏蔽回归通过生产 matcher 与持久化设置覆盖图片、链接、文字、艾特文字、所有消息、禁用/缺失模板、名单关闭、排除、默认规则及选择器重选配置保留，共 56 项断言；修改前“模板禁用但名单有独立文字规则”可稳定复现失败。测试使用 `JSON_JAR=<org.json.jar> node scripts/run_message_block_tests.cjs`，不运行 Gradle。用户反馈的具体群聊尚无同条消息样本及设备验证，不能据静态测试宣称所有漏消息均已解决。关键词通知按既有关键词与 `@我/@所有人` 规则放行的行为保持不变。
...
- 开启后，每个非空分组在微信本地建立确定性的 `wxid_hchat_group_*` 虚拟联系人和聚合会话，子分组虚拟入口挂到父分组。普通好友和群聊继续按原机制更新真实会话 `parentRef`；公众号（`gh_`、联系人 `verifyFlag!=0`，或已知原生公众号父级）只在列表中投影，不再写入模块父级，避免干扰微信的订阅号/服务号更新。已被旧版本归组的公众号仅按当前账号保存的可信原始值恢复；原始值缺失、损坏或仍指向模块父级时停止该项恢复并记录错误，不擅自清空。归属读取失败时停止同步；恢复前先提交原始映射，恢复失败不删除记录，也不清理仍可能被引用的虚拟父级。关闭分组功能时仍允许修复旧公众号归属，普通会话原有停同步行为保留。
...
- 虚拟分组原生二级页面右上角通过各版本稳定存在的 `MMFragment#addTextOptionMenu(int,String,OnMenuItemClickListener)` 增加 `菜单`。菜单提供 Hchat 模块入口、所有消息标为已读、批量删除消息、开启/解除消息免打扰、批量发送文字、图片、语音、视频、表情、文件、收藏、名片或 XML、发送群聊邀请、添加会话、移出、移至、搜索和分组显示设置；显示设置包含主页置顶、自定义头像、命名、分组内会话固定排序、同级分组位置、三态未读数模式、最新消息预览和空分组显示。操作范围默认使用页面当前生效的会话集合，包含手动加入和自动归拢的会话，并递归包含全部子分组；只有移出和移至处理当前分组直属手动会话。`添加会话` 打开的 `选择分组会话` 选择器使用通用分类，支持好友、群聊、公众号、标签和全部，也可按已有聊天分组及其子分组筛选真实会话；进入标签分类后可按具体好友标签筛选，搜索和全选/取消全选仅作用于当前分类、标签及聊天分组筛选的交集。会话数据在后台读取；读取成功、失败、取消或 Activity 销毁时都先关闭加载层，成功时再通过 decor 的下一帧回调打开选择器，避免透明全屏遮罩残留并吞掉触摸。批量删除消息先多选并二次确认，再提交微信 `MsgInfoStorageLogic` 的 Stage1/Stage2 原生批量清理入口，不直接 SQL 删除消息；该入口在 8.0.49 至 8.0.74 为 `(List,callback,long)`，8.0.76 为 `(List,callback)`，使用 `summerdel deleteMsgByTalker` 与 `AsyncDeleteMessageStage1` 锚点定位并写入运行时版本缓存。私聊免打扰使用微信 `setMute/unSetMute` 原生联系人入口，群聊使用 RoomSDK 的 `notifyMsg=0/1` 操作并调用原生 `factory.a#b()` 提交；运行时只缓存定位结果，不按混淆类名分支。批量发送和邀请在后台单线程按有界小批次执行，邀请目标群聊支持多选和全选，并按“好友数 × 群数”统计已提交请求；目标间保留间隔，不在主线程一次提交全部目标；收藏发送结果表示请求已提交，语音收藏仍可能继续等待微信下载。
- 首页查询入口已横向确认：8.0.49 为 `com.tencent.mm.storage.v4#D`，8.0.58 为 `a5#D`，8.0.66/8.0.68 为 `n3#D`，8.0.72 为 `m4#D`，8.0.74 为 `m4#C`，8.0.76 为 `l4#C`，签名均为 `(int,List,String,boolean,String):Cursor`。高版本首页适配器会读取微信内部会话对象，因此虚拟 wxid 必须同时存在于 `rcontact` 与 `rconversation`。8.0.49 与 8.0.76 的导出实现确认第三个参数只控制是否追加根 `parentRef` 条件，并不表示目标父级；当前实现沿用原生首页中的虚拟分组行并重新排序；仅从首页结果中过滤已成功发布到分组快照的公众号投影。原生父级更新入口用于恢复旧公众号归属及普通会话归拢，公众号不再通过该入口写入模块父级。
- 公众号分组的原生二级列表在原查询完成后，按本分组直属、当前账号生效的公众号 ID 从 `rconversation` 读取补充行；不展开其他组或子组的真实会话。补充查询继承用户名排除清单，按原列映射、按 `username` 去重，以较新的补充行覆盖旧行，再统一按 `flag DESC, conversationTime DESC` 排序并应用原条数上限，最后执行用户设置的组内置顶/置底/固定顺序。原列表为空时也能显示纯公众号分组。此路径只处理已核验的原生过滤参数 `0`，其他过滤模式不擅自放宽。成员关系变化后发布账号隔离快照，再通知虚拟分组行并刷新打开的分组适配器，避免计数/未读相同但成员替换时仍显示旧列表。
- DexClub 核验 8.0.49/58/66/68/72/74/76/77/78 的原生二级查询分别为 `storage.v4.t/a5.s/n3.s/n3.s/m4.s/m4.s/l4.s/l4.s/l4.s`，共同签名 `(int,List,String,int):Cursor`，投影均为 `SELECT * FROM rconversation`，父级条件按第三参、排除项按第二参、上限按第四参；49/58/78 的附加过滤辅助方法已确认 `a(false,0)` 返回空条件。本次不更换 Hook 定位或新增混淆类名分支，复用已有缓存入口。
- 这项修复针对“公众号自动归拢会覆盖原生归属”这一已确认的代码问题；未在反馈设备上复现新文章接收。不会删除、补造或搬移消息记录，也不保证找回此前没有落库的推送。父级决策 160 项、Cursor 合并 43 项、从实际 Runtime 提取的保存/恢复接入逻辑 71 项回归均通过；分别运行 `node scripts/run_conversation_group_parent_tests.cjs`、`node scripts/run_conversation_group_official_tests.cjs`、`JSON_JAR=<org.json.jar> node scripts/run_conversation_group_parent_runtime_tests.cjs`。接入回归覆盖原始记录提交失败后的重试、恢复失败不删除备份、损坏 JSON、明确空父级、账号切换及分批恢复。未运行 Gradle、未打包，真机新推送验证仍需后续执行。
...
Agent 配置支持多份模型配置档案，可切换、新建、重命名和删除，并可对当前聊天接口执行最小请求测试连接。聊天输入区显示当前配置和模型，可直接切换；运行中切换只影响下一次请求，不改变已经启动的请求。输入区快捷选项可单独开关联网搜索和每个已配置的 MCP 服务器，关闭联网搜索后 Agent 不会发起搜索请求；“插件文件修改确认”可选择“每次询问”或“始终允许”，前者确认每次工具写入和最终提交，后者跳过全部确认并自动提交；“提示缓存”可选择“自动”“强制”或“关闭”，新配置和缺少该字段的旧配置默认使用“强制”，并按模型配置档案独立保存；用户已明确保存的选择不被覆盖。新建配置从空白开始：API 地址、密钥、模型 ID 和 MCP 列表均为空，接口类型及推理强度等选项恢复初始默认值，不复制当前配置；原配置保留，切换回去可继续使用。接口类型按配置档案独立保存：`OpenAI 兼容`、OpenAI、OpenRouter 和硅基流动会补全 `/v1/chat/completions`，DeepSeek 会补全 `/chat/completions`，Anthropic 会补全 `/v1/messages`，Gemini 会按当前模型和流式状态生成 `/v1beta/models/{model}:generateContent` 或 `:streamGenerateContent?alt=sse`。地址规范化会识别尚未输入完整的结尾并保留反向代理路径前缀，例如 `https://xapi.deuo.top//v1/chat/completi` 会解析为 `https://xapi.deuo.top/v1/chat/completions`；选择具体平台时会填入官方地址，之后也可改为平台代理地址并按该平台规则补全。Anthropic 使用 Messages API 原生消息与工具协议，Gemini 使用 generateContent 原生内容与函数调用协议，其余平台使用 Chat Completions 协议。只有 `自定义请求链接` 要求用户填写完整 HTTP(S) URL，客户端仅去除首尾空白，不补全或改写路径与查询参数。页面实时显示最终请求地址。旧版单配置中的 `apiBaseUrl` 和 `apiPath` 首次读取时自动合并到“默认配置”，并继续回写旧键以兼容降级读取。

聊天输入区的模型入口与 Agent 配置页都支持直接填写模型 ID，无需先拉取模型列表，也不要求模型出现在列表中。模型选择页通过“获取模型列表”按需拉取；空列表、接口失败或搜索结果为空均不影响手动输入与应用，拉取结果不会覆盖正在输入的模型 ID。点击“应用模型设置”会去除模型 ID 首尾空白并拒绝空值，列表中单选只更新待应用项，不自动退出页面。

聊天页的快捷选项、模型选择页和 Agent 配置页均提供“推理强度”，按模型配置档案保存。聊天页快捷选项中选择档位后立即保存并用于下一次请求，无需进入配置页；选项较多时可滚动查看。旧配置缺少该字段时使用“默认”，不额外发送推理参数；非默认档位按接口协议发送，并只提供当前已适配模型的档位。兼容接口的实际能力仍由模型和服务端决定，不支持时显示接口错误，不会静默去掉推理参数后假装成功。配置切换会恢复各自的推理强度；运行中应用模型设置只作用于后续请求，当前已启动的请求保持原配置。测试连接、正常聊天与上下文压缩复用相同的参数适配入口。接口适配中，OpenAI 兼容接口发送 `reasoning_effort`，OpenRouter 发送 `reasoning.effort`，Anthropic 发送 `output_config.effort`，Gemini 发送 `generationConfig.thinkingConfig.thinkingLevel`；DeepSeek 和硅基流动仅对已核验模型组合相应思考开关与 effort。Anthropic effort 控制回答与推理投入，不额外开启需要签名回放的思考块；OpenRouter Claude 因当前未保存完整签名推理历史，暂只提供默认档位。原生接口的未知模型保留默认，不猜测 token 预算或扩展档位。
...
以上为 2026-09-11 的历史记录；当前 `alt-entry` 已改为仅支持 `arm64-v8a` 和 64 位微信进程，ABI 支持范围以本文当前 Gradle 接入约定为准。
...
## 消息显示时间：排除系统提示与通话记录（2026-09-24）

- “消息显示时间”仅为普通聊天消息添加 Hchat 标签；邀请入群等系统提示、拍一拍、撤回提示和通话记录不再添加该标签。微信原有的提示内容、时间分隔和消息数据保持不变；普通文字提及“撤回”“拍一拍”“通话”等词不会被过滤。
- `MessageDetailsVisibility` 按完整消息类型判断，在读取详情、查找时间控件和布局锚点之前跳过排除项。统一插入入口也执行相同判断，覆盖设置刷新、布局恢复和挂载回调；排除时取消延迟任务、使旧绑定失效、移除 Hchat 标签并恢复头像预留间距，不进入布局失败重试。删除原来专门为系统/拍一拍行定位居中时间的分支及资源 ID 缓存。
- DexClub 静态核验的系统类型并集为 `10000, 10002, 570425393, 64, 603979825, 889192497, 922746929, 268445456, 268445458, 285222674, -1879048191, 1077936177`。其中 `285222674` 自本次核验的 8.0.66 起出现在系统谓词；其余在全部八版一致。使用精确整数比较，不将低位相同的普通卡片一起排除。
- 系统谓词：8.0.49 `storage.k9.K1()`、8.0.58 `storage.q9.b2()`、8.0.66 `storage.f8.h2()`、8.0.68 `storage.f8.i2()`、8.0.72/74 `storage.f9.j2()`、8.0.76/77 `storage.e9.j2()`，类名前缀均为 `com.tencent.mm`，返回 `boolean`。八版原生通话谓词均核验出 `50/53` 与 `52`；同时沿用公共消息模型及消息文本颜色功能已记录的 `1000052/1000053` 语音/视频通话类型。未经本次确认的 `51` 不额外扩入过滤范围。
- `node scripts/run_message_type_labels_tests.cjs` 覆盖系统/拍一拍/通话排除，普通文字、语音、图片、卡片、高位消息、有符号礼物及未知类型保留，共 96 项断言通过。本次未运行 Gradle、未打包，真实聊天列表与滚动复用仍需设备验证。

## 语音转发与静默红包兼容（8.0.77 / 8.0.78）

微信启用新语音文件存储后，消息的旧 `imgPath` 可以为空。聊天语音转发与通用转发快照优先把已有原生消息交给 `WeChatVoiceApi.resolvePath(nativeMessage, fileName)`，由微信服务解析新旧存储路径；旧入口仍用于没有新服务的版本。定位使用 `[f11]getVoiceFullPath, businessType: ` 锚点和实例 `(消息对象,String,boolean)->String` 签名，结果进入现有微信版本、热更新及 ClassLoader 指纹缓存；旧版锚点不存在也记录为已检查，缓存描述符失效则重新定位。

8.0.78 的完成录音方法增加第五个 `msgSource` 参数。公共语音发送同时支持三、四、五参数版本，新增参数按微信自身复制语音调用传 `null`。新增路径解析与完成语音记录的异常记录版本和目标签名；转发结果以实际发送返回值为准，不能仅凭网络队列已就绪报告成功。

DexClub 横向核验如下，表内混淆名称仅作 APK 证据，运行时代码使用动态定位：

| 微信版本 | 完成录音参数数 | 按原生消息取语音路径 | 普通收包 / 拆包类 |
| --- | --- | --- | --- |
| 8.0.49 | 3 | 锚点未命中，使用旧路径 | `m5 / g5` |
| 8.0.58 | 4 | 锚点未命中，使用旧路径 | `t5 / n5` |
| 8.0.66 | 4 | `c50.b.ai` | `i6 / c6` |
| 8.0.68 | 4 | `z50.b.Fh` | `i6 / c6` |
| 8.0.72 | 4 | `m80.b.Ui` | `i6 / c6` |
| 8.0.74 | 4 | `k90.b.yj` | `l6 / f6` |
| 8.0.76 | 4 | `h90.b.Lj` | `l6 / f6` |
